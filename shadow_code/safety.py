...
_DESTRUCTIVE_PATTERNS: list[tuple[re.Pattern, str]] = [
    (
        re.compile(
            r"\brm\s+.*-[a-zA-Z]*r[a-zA-Z]*f\b|\brm\s+.*-[a-zA-Z]*f[a-zA-Z]*r\b|\brm\s+-rf\b"
        ),
    ...
    (re.compile(r"\bmkfs\b"), "Format filesystem (mkfs)"),
    ...
    (re.compile(r"\bsudo\s+rm\b"), "Privileged delete (sudo rm)"),
]
